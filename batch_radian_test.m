function results = batch_radian_test()
    close all;
    consts = get_consts();
    angles = [0.5 1.0 1.3 1.5 2.0];
    ...
    for k = 1:length(angles)
        ...
        fprintf('angle=%5.2f rad (%6.1f deg)  t=%6.2f  J=%7.2f  y=%9.2f  z=%7.2f  th=%8.3f deg  speed=%7.2f  dth=%8.3f deg/s\n', angles(k), angles(k)*180/pi, t(end), J, x(end,1), x(end,2), x(end,3)*180/pi, norm(x(end,5:6)), x(end,7)*180/pi);
    end
end
