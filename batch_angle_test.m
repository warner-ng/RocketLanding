function results = batch_angle_test()
    close all;
    consts = get_consts();
    angles = [1.3 1.5 2 5 10 20];
    results = zeros(length(angles), 8);
    for k = 1:length(angles)
        x0 = [50; 550; angles(k)*pi/180; 0; 0; 0; 0; 0; consts.m_nofuel + consts.max.m_fuel];
        ...
        fprintf('angle=%5.1f deg  t=%6.2f  J=%7.2f  y=%8.2f  z=%7.2f  th=%8.3f deg  speed=%7.2f  dth=%8.3f deg/s\n', results(k,:));
    end
end

function [dx, u] = odefun_local(t, x, wind, consts, ctrl)
    y = x(1); z = x(2); th = x(3); psi = x(4);
    dy = x(5); dz = x(6); dth = x(7); dpsi = x(8); m = x(9);
    ...
    dx = [f_vec + g_vec*u + d_vec; integrator_dx];
end

function [value, isterminal, direction] = odeevents_touchdown_local(t, x, wind, consts, ctrl)
    z = x(2); th = x(3); L = consts.L; r = consts.r;
    contact_points = [z-L*cos(th)-r*sin(th); z-L*cos(th)+r*sin(th); z+L*cos(th); z-r*sin(th); z+r*sin(th)];
